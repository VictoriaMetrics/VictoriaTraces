...
func RequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	if cp.IsTimeFieldSet {
		unsupportedOptionsLogger.Warnf("/internal/insert endpoint doesn't support setting time fields via _time_field query arg and via VT-Time-Field request header; "+
	...
	if len(cp.MsgFields) > 0 {
		unsupportedOptionsLogger.Warnf("/internal/insert endpoint doesn't support setting msg fields via _msg_field query arg and via VT-Msg-Field request header; "+
			"ignoring them; msgFields=%q", cp.MsgFields)
		cp.MsgFields = nil
	}
	if len(cp.StreamFields) > 0 {
		unsupportedOptionsLogger.Warnf("/internal/insert endpoint doesn't support setting stream fields via _stream_fields query arg and via VT-Stream-Fields request header; "+
			"ignoring them; streamFields=%q", cp.StreamFields)
		cp.StreamFields = nil
	}
	if len(cp.DecolorizeFields) > 0 {
		unsupportedOptionsLogger.Warnf("/internal/insert endpoint doesn't support setting decolorize_fields query arg and VT-Decolorize-Fields request header; "+
