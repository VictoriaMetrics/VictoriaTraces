...
export class TracePageImpl extends React.PureComponent<TProps, TState> {
  ...
  ensureTraceFetched() {
    ...
    if (id && id !== id.toLowerCase()) {
      const nextLocation = getLocation(id.toLowerCase(), location.state as Record<string, string> | TNil);
  ...
  render() {
    ...
    const locationState = location.state as LocationState | TNil;
...
export function mapStateToProps(state: ReduxState, ownProps: TOwnProps): TReduxProps {
  ...
  const { config, embedded, router } = state;
  ...
  const locationState = router.location.state as LocationState | TNil;
