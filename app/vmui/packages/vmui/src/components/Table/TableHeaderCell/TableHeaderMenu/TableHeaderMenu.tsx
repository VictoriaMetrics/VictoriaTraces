...
const TableHeaderMenu = <T extends object>(props: TableHeaderMenuProps<T>) => {
  ...
  const createHandlerKeyDownByMenu = (callback?: () => void) => (e: KeyboardEvent) => {
        ...
        >
          {sections.map((options, idx) => (
            <div
              ...
              // eslint-disable-next-line @eslint-react/no-array-index-key -- sections has a fixed structural order (display/text/size), only conditionally filtered for visibility, never reordered
              key={idx}
            >
              {options.map((option) => (
                <div
                  ...
                  onKeyDown={createHandlerKeyDownByMenu(option.onClick)}
                  role="menuitem"
                  tabIndex={0}
                >
