...
export const useTableColumnPrefs = (tableKey: string) => {
  ...
  const syncPrefs = useCallback(() => {
    ...
    // eslint-disable-next-line @eslint-react/set-state-in-effect -- called from an effect to load persisted column prefs from storage on mount/key change, and from a storage event listener
    setColumnPrefs(next);
  }, [storageKey]);
