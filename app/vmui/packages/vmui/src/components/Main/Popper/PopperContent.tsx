...
const PopperContent: FC<PopperContentProps> = ({
  ...
  const popperStyle = useMemo(() => {
    ...
    return position;
    // anchorSize isn't read in the body; buttonPos comes from buttonRef.current.getBoundingClientRect() each run, so these deps only re-trigger the recompute when the anchor resizes
