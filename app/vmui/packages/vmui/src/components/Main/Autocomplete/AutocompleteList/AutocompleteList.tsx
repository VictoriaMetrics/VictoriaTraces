import { FC, Fragment, useCallback, useEffect, useRef, useState } from "preact/compat";
...
const AutocompleteList: FC<Props> = ({
  ...
  const [listId] = useState(() => `vm-autocomplete-list-${Math.random().toString(36).slice(2)}`);
  ...
  useEffect(scrollToValue, [focusOption, options, getOptionId]);
