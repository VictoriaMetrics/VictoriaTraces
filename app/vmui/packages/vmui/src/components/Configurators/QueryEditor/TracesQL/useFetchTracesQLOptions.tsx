...
type FetchDataArgs = {
  urlSuffix: string;
  setter: (value: TracesFiledValues[]) => void;
...
export const useFetchTracesQLOptions = (contextData?: ContextData) => {
  const { serverUrl } = useAppState();
  const { period: { start, end } } = useTimeState();
  const { autocompleteCache } = useTracesState();
  ...
  const processData = (values: TracesFiledValues[], type: ContextType): AutocompleteOptions[] => {
    return values.map(v => ({
      value: v.value,
      type: `${type}`,
      icon: icons[type]
  ...
  const fetchData = async ({ urlSuffix, setter, params }: FetchDataArgs) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const value = (data?.values || []) as TracesFiledValues[];
        setter(value || []);
  ...
  useEffect(() => {
    ...
    const setter = (filterNames: TracesFiledValues[]) => {
  ...
  useEffect(() => {
    ...
    const setter = (filterValues: TracesFiledValues[]) => {
