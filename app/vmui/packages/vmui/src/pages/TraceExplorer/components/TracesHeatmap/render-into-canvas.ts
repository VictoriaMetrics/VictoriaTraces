import { HEATMAP_AXIS_FONT, HEATMAP_ERROR_CELL_THRESHOLD } from "./constants";
...
/**
 * Renders the heatmap - density cells and both axes - into a single canvas,
...
export default function renderIntoCanvas(params: RenderHeatmapParams): RenderHeatmapResult {
  const {
    canvas, counts, errors, maxCount, cellColor, errorColor, axisColor,
  ...
  for (let col = 0; col < columns; col++) {
    for (let row = 0; row < rows; row++) {
      ...
      ctx.fillStyle = (errors[col]?.[row] ?? 0) > HEATMAP_ERROR_CELL_THRESHOLD ? errorColor : cellColor;
