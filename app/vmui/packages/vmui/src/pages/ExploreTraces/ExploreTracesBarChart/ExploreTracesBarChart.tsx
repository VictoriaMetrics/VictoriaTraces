...
const ExploreTracesBarChart: FC<Props> = ({ traceHits, period, error, isLoading, onApplyFilter }) => {
  const { isMobile } = useDeviceDetect();
  const timeDispatch = useTimeDispatch();
  const [searchParams] = useSearchParams();
  const hideChart = useMemo(() => searchParams.get("hide_chart"), [searchParams]);

  const getYAxes = (traceHits: TraceHits[], timestamps: number[]) => {
    return traceHits.map(hits => {
  ...
  const data = useMemo(() => {
    if (!traceHits.length) return [[], []] as AlignedData;
    const xAxis = generateTimestamps(dayjs(traceHits[0].timestamps[0]));
    const yAxes = getYAxes(traceHits, xAxis);
  ...
  return (
    ...
    >
      ...
      {!error && noDataMessage && (
        <div className="vm-explore-traces-chart__empty">
...
export default ExploreTracesBarChart;
