...
import { getTracesUrl } from "../../../api/traces";
import { ErrorTypes, TimeParams } from "../../../types";
import { Traces } from "../../../api/types";
import dayjs from "dayjs";
import { useTenant } from "../../../hooks/useTenant";

export const useFetchTraces = (server: string, query: string, limit: number) => {
  ...
  const url = useMemo(() => getTracesUrl(server), [server]);
  ...
  const fetchTraces = useCallback(async (period: TimeParams) => {
    ...
    try {
      ...
      const data = text.split("\n", limit).map(parseLineToJSON).filter(line => line) as Traces[];
      setTraces(data);
      return true;
    } catch (e) {
...
const parseLineToJSON = (line: string): Traces | null => {
  try {
