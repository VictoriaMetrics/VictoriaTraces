...
import GroupTracesHeaderItem from "./GroupTracesHeaderItem";
import { TRACES_GROUP_BY, TRACES_URL_PARAMS, WITHOUT_GROUPING } from "../../../constants/traces";
import { GroupTracesType } from "../../../types";

interface Props {
  group: GroupTracesType;
  index: number;
}

const GroupTracesHeader: FC<Props> = ({ group, index }) => {
  ...
  const compactGroupHeader = searchParams.get(TRACES_URL_PARAMS.COMPACT_GROUP_HEADER) === "true";
  ...
  const calcVisiblePairsCount = useCallback(() => {
    ...
    const selector = ".vm-group-traces-section-keys__pair:not(.vm-group-traces-section-keys__pair_more)";
  ...
  return (
    <div
      className={classNames({
        "vm-group-traces-section-keys": true,
        "vm-group-traces-section-keys_compact": compactGroupHeader,
      })}
      ref={containerRef}
    >
      <span className="vm-group-traces-section-keys__title">
        {groupBy === WITHOUT_GROUPING ? WITHOUT_GROUPING : <>{index + 1}. Group by <code>{groupBy}</code>:</>}
      </span>
      {pairs.map((pair, i) => (
        <GroupTracesHeaderItem
      ...
      {hideParisCount > 0 && (
        <>
          <div
            className={classNames({
              "vm-group-traces-section-keys__pair": true,
              "vm-group-traces-section-keys__pair_more": true,
              "vm-group-traces-section-keys__pair_dark": isDarkTheme
            })}
            ref={moreRef}
          ...
          >
            <div className="vm-group-traces-section-keys vm-group-traces-section-keys_popper">
      ...
      <span className="vm-group-traces-section-keys__count">{group.total} entries</span>
    </div>
  )
  ;
};

export default GroupTracesHeader;
