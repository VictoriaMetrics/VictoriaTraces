...
import { TRACES_GROUP_BY, TRACES_URL_PARAMS } from "../../../constants/traces";
...
const GroupTracesFieldRow: FC<Props> = ({ field, value, hideGroupButton }) => {
  ...
  const displayFieldsString = searchParams.get(TRACES_URL_PARAMS.DISPLAY_FIELDS) || "";
  ...
  const handleSelectDisplayField = () => {
    ...
    searchParams.set(TRACES_URL_PARAMS.DISPLAY_FIELDS, newDisplayFields.join(","));
    setSearchParams(searchParams);
  };

  const handleSelectGroupBy = () => {
    isGroupByField ? searchParams.delete(TRACES_URL_PARAMS.GROUP_BY) : searchParams.set(TRACES_URL_PARAMS.GROUP_BY, field);
    setSearchParams(searchParams);
  };
  ...
  return (
    <tr className="vm-group-traces-row-fields-item">
      <td className="vm-group-traces-row-fields-item-controls">
        <div className="vm-group-traces-row-fields-item-controls__wrapper">
          <Tooltip title={copied ? "Copied" : "Copy to clipboard"}>
            <Button
              className="vm-group-traces-row-fields-item-controls__button"
              variant="text"
              color="gray"
              size="small"
              startIcon={<CopyIcon/>}
      ...
      <td className="vm-group-traces-row-fields-item__key">{field}</td>
      <td className="vm-group-traces-row-fields-item__value">{value}</td>
    </tr>
  );
};

export default memo(GroupTracesFieldRow);
