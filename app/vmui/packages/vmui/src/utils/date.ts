...
export const STANDARD_CLOCK_TIME_FORMAT = "HH:mm:ss.SSS";
...
/**
 * @param {number} duration - Unix Time
 * @return {string} formatted clock time with millisecond precision, no date
 *
 * @example
 * ```
 * formatClockTime(0) // => 00:00:00.000
 * ```
 */
export function formatClockTime(duration: number): string {
  return dayjs(duration / ONE_MILLISECOND).format(STANDARD_CLOCK_TIME_FORMAT);
}
